...
Data construction and data loader functions
"""
...
def build_hand_dataset(yaml_file, args, is_train=True, scale_factor=1):
    ...
    if not op.isfile(yaml_file):
        yaml_file = op.join(args.dataset_dir, yaml_file)
...
def make_hand_data_loader(args, yaml_file, is_train=True, start_iter=0, scale_factor=1):
    ...
    if is_train==True:
        shuffle = True
        iters_per_batch = len(dataset) // args.batch_size
        num_iters = iters_per_batch * args.num_train_epochs
        print("DATA_LOADER", "Train with {} images per batch.".format(args.batch_size))
        print("DATA_LOADER", "Total training steps {}".format(num_iters))
    else:
        shuffle = False
        num_iters = None
        start_iter = 0
